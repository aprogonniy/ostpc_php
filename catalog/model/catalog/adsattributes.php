...
class ModelCatalogAdsattributes extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (isset($data['filter_name']) && $data['filter_name']) {
			if (isset($data['filter_description']) && $data['filter_description']) {
				...
				$sql .= " AND (LCASE(pd.name) LIKE '%" . $this->db->escape(mb_strtolower($data['filter_name'], 'utf-8')) . "%' OR p.product_id IN (SELECT pt.product_id FROM " . DB_PREFIX . "product_tag pt WHERE pt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pt.tag LIKE '%" . $this->db->escape(mb_strtolower($data['filter_name'], 'utf-8')) . "%'))";
			}
		}

		if (isset($data['filter_tag']) && $data['filter_tag']) {
			$sql .= " AND p.product_id IN (SELECT pt.product_id FROM " . DB_PREFIX . "product_tag pt WHERE pt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND LOWER(pt.tag) LIKE '%" . $this->db->escape(mb_strtolower($data['filter_tag'], 'utf-8')) . "%')";
		}
	...
	public function getCategories1($product_id) {
	...
	public function getTotalProducts($data = array()) {
		...
		if (isset($data['filter_tag']) && $data['filter_tag']) {
			$sql .= " AND p.product_id IN (SELECT pt.product_id FROM " . DB_PREFIX . "product_tag pt WHERE pt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pt.tag LIKE '%" . $this->db->escape(mb_strtolower($data['filter_tag'], 'utf-8')) . "%')";
		}
        ...
        $sql .= "  (SELECT  count(*) FROM " . DB_PREFIX . "product p
			...
			if ($filter_sub_category == true) { 
				...
				$categories = $this->getCategoriesByParentId($filter_category_id);
                                $implode_data= implode(',',$categories);
				$sql .= " AND p.product_id IN (SELECT p2cc.product_id FROM " . DB_PREFIX . "product_to_category p2cc WHERE p2cc.category_id in(" . $implode_data . "))";
			} else {
				$sql .= " AND p.product_id IN (SELECT p2cc.product_id FROM " . DB_PREFIX . "product_to_category p2cc WHERE p2cc.category_id = '" . (int)$filter_category_id . "')";
			}
        $sql .= ")  AS countproduct ";
        
        $sql .= " FROM " . DB_PREFIX . "manufacturer m ";
            $sql .= "  WHERE m.manufacturer_id in(SELECT pm.manufacturer_id FROM " . DB_PREFIX . "product pm
                           WHERE pm.status = '1' ";
			if ($filter_sub_category == true) {
				$sql .= " AND pm.product_id IN (SELECT p2c.product_id FROM " . DB_PREFIX . "product_to_category p2c WHERE p2c.category_id in(" . $implode_data . "))";
			} else {
				$sql .= " AND pm.product_id IN (SELECT p2c.product_id FROM " . DB_PREFIX . "product_to_category p2c WHERE p2c.category_id = '" . (int)$filter_category_id . "')";
			}
	...
	public function getGroupsAttribute($filter_category_id = 0, $filter_sub_category = '', $filter_manufacturer_id='') {
            if($filter_sub_category != '' OR $filter_category_id == 0) {
                    $category_id = $this->getCategoriesByParentId($filter_category_id);
                } else {
                   $category_id[0] = $filter_category_id; 
                }
            ...
            $sql = "SELECT *,
               ...
               if($filter_manufacturer_id != '') {               
                  $sql .= " AND p1.manufacturer_id =" . (int)$filter_manufacturer_id;
               } 
                    $sql .= ") AS countproduct
               ...
               if($filter_manufacturer_id != '') {               
                  $sql .= " AND p.manufacturer_id =" . (int)$filter_manufacturer_id;
               } 
	...
	public function getAttributes($filter_category_id = 0, $filter_sub_category = '', $filter_manufacturer_id = '', $filter_groups = '') {
                   if($filter_sub_category != '' OR $filter_category_id == 0) {
                            $category_id = $this->getCategoriesByParentId($filter_category_id);
                        } else {
                           $category_id[0] = $filter_category_id; 
             ...
             if($filter_groups != '') {
                        $sql .= "  AND a1.attribute_group_id = '" . (int)$filter_groups  . "'"; 
                }                       
                    $sql .= ") AS countproduct
            ...
            if($filter_groups != '') {
                        $sql .= "  AND a.attribute_group_id = '" . (int)$filter_groups  . "'"; 
                }            
    ...
    public function getAutocomplete($q = '') {
        $sql = "SELECT p.name FROM " . DB_PREFIX . "product_description p WHERE
            p.language_id = '" . (int)$this->config->get('config_language_id') . "' AND 
            LCASE(p.name) LIKE '" . $this->db->escape(mb_strtolower($q, 'UTF-8')) . "%'";
